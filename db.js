...
db.serialize(() => {
  ...
  // Ajout des colonnes optionnelles (ignore les erreurs si déjà présentes)
  db.run(`ALTER TABLE messages ADD COLUMN sender_jid TEXT`, err => {/* noop si existe déjà */ });
  db.run(`ALTER TABLE messages ADD COLUMN replied_to_message_id TEXT`, err => {/* noop si existe déjà */ });
  db.run(`ALTER TABLE messages ADD COLUMN sources_json TEXT`, err => {/* noop si existe déjà */ });
  db.run(`ALTER TABLE messages ADD COLUMN coherence_json TEXT`, err => {/* noop si existe déjà */ });
  db.run(`ALTER TABLE messages ADD COLUMN needs_review INTEGER DEFAULT 0`, err => {/* noop si existe déjà */ });
